...
{if $post->comments}
	...
	<h2 id="comments-title">
		{_n 'Review', 'Reviews', $post->commentsCount} ({$post->commentsCount})
	</h2>
			...
			<article id="comment-{$comment->id}" class="comment">
				...
				<div class="body clearfix">
					...
					<div class="content ">
						...
						{if !$comment->approved}
							<em class="comment-awaiting-moderation">{__ 'Your review is awaiting moderation.'}</em><br>
						{/if}
						{!$comment->content}
					</div>
				</div>
...
{elseif !$post->hasOpenComments && $post->type != 'page' && $post->hasSupportFor('comments')}
	...
	<p class="nocomments">{__ 'Reviews are closed.'}</p>

{/if}

{* translations for parameters *}

{capture $reviewNoun}{_x 'Review', 'noun'}{/capture}
{capture $reviewLoggedIn}{!__ 'You must be <a href="%s">logged in</a> to post a review.'|printf: wp_login_url(apply_filters('the_permalink', get_permalink()))}{/capture}
{capture $reviewLeave}{__ 'Leave a Review'}{/capture}
{capture $reviewReplyTo}{__ 'Leave a Review to %s'}{/capture}
{capture $reviewCancel}{__ 'Cancel review'}{/capture}
{capture $reviewPost}{__ 'Post Review'}{/capture}
...
{else}
	<p class="nopassword">{__ 'This post is password protected. Enter the password to view any reviews.'}</p>
{/if}
</div><!-- #comments -->
